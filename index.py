    ...
    *,
    ...
    def checkbox(item):
        ...
        def on_click(event):
            if (
                event["new"] is True
            ):  # Do not refactor to `if event['new']:`. It will fail.
                item.checked = True
    ...
    if items:
        ...
        def check_all_on_click(event):
            ...
            if event["name"] == "_property_lock" and event["new"].get("value") is True:
                ...
                event["name"] == "_property_lock" and event["new"].get("value") is False
            ):
                for box in check_boxes:
                    box.value = False
...
def render():
    ...
    display(
        widgets.HTML(
            f"""<p>Check the project you use.<br/>
...
Think of it as a preset. Of course you can override each path manually.</p>"""
    ...
    display(
        widgets.HTML(
            f"<p>Specity the root directory of the project (e.g. <b>stable-diffusin-wedui</b> or <b>ComfyUI</b>).</p>"
        )
    )
    root_text_input = widgets.Text(
        value="/workspace/stable-diffusion-webui/",
        layout=widgets.Layout(width="92%"),
    )
    ...
    textarea_desc += (
        "<b>Multiple URLs should be separated by newlines (not comma).</b> "
    )
    textarea_desc += (
        "Note that any words following after <b>##</b> is treated as comment. "
    )
    textarea_desc += (
        "So ignored when parsing URL, but useful for taking memo (e.g. model name). "
    )
...
https://civitai.com/models/33918 ## Shampoo Mix latest version""",
    ...
    textual_inversion_list = [
        ...
        Item(
            display_name="EasyNegative.safetensors",
            file=File(
        ...
        Item(
            display_name="negative_hand-neg.pt",
            file=File(
        ...
        Item(
            display_name="ng_deepnegative_v1_75t.pt",
            file=File(
    ...
    controlnet_v1_1_list = [
        ...
        for display_name, filename in [
            ...
            (
                "v11p_sd15s2_lineart_anime (1.45 GB)",
    ...
    t2i_adapter = section(
        title="T2I-Adapter", default_path="models/ControlNet/", items=t2i_adapter_list
    )
...
def parse_civitai_url(url):
    ...
    if parsed_query.get(
        "modelVersionId"
    ):  # For URLs like https://civitai.com/models/20282?modelVersionId=58992
        ...
        get_model = lambda result: result["modelVersions"][
            0
        ]  # get a model of specific(the latest) version
    ...
    for section in sections.values():
        ...
        if section.textarea:
            items_from_textarea = parse_textarea(
                textarea=section.textarea, dest=section.path.value.strip()
            )
